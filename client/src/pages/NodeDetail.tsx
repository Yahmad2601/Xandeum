...
import { ArrowLeft, Server, HardDrive, Cpu, Clock, MapPin, Share2 } from "lucide-react";
import {
  AreaChart,
  Area,
  XAxis,
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-card/40 border border-white/5 p-4 rounded-xl">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          <section className="md:col-span-2 glass-panel p-6 rounded-2xl">
            <h3 className="font-bold mb-4">Uptime History</h3>
            <div className="h-40">
              <Sparkline data={node.uptimeHistory || []} color="#10b981" height={160} />
            </div>
            <div className="mt-4 grid grid-cols-24 gap-px opacity-50">
               {/* Just visual blocks representing hours */}
               {Array.from({length: 24}).map((_, i) => (
                 <div key={i} className="h-2 bg-emerald-500/50 rounded-sm" title={`Hour ${i}`} />
               ))}
            </div>
          </section>

          <section className="glass-panel p-6 rounded-2xl">
            <h3 className="font-bold mb-4">Raw Metadata</h3>
